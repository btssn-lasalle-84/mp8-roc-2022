...
mysqli_select_db($conn,$nomDeBase );
  $req = mysqli_query($conn, 'SELECT lundi FROM graphique WHERE idPoubelle=1');
  $donnees = mysqli_fetch_array($req) ; 
  $nombreDeSacLundi = $donnees['lundi'];
  
  $req2 = mysqli_query($conn, 'SELECT mardi FROM graphique WHERE idPoubelle=1');
  $donnees2 = mysqli_fetch_array($req2) ; 
  $nombreDeSacMardi = $donnees2['mardi'];

  $req3 = mysqli_query($conn, 'SELECT mercredi FROM graphique WHERE idPoubelle=1');
  $donnees3 = mysqli_fetch_array($req3) ; 
  $nombreDeSacMercredi = $donnees3['mercredi'];

  $req4 = mysqli_query($conn, 'SELECT jeudi FROM graphique WHERE idPoubelle=1');
  $donnees4 = mysqli_fetch_array($req4) ; 
  $nombreDeSacJeudi = $donnees4['jeudi'];

  $req5 = mysqli_query($conn, 'SELECT vendredi FROM graphique WHERE idPoubelle=1');
  $donnees5 = mysqli_fetch_array($req5) ; 
  $nombreDeSacVendredi = $donnees5['vendredi'];

  $req6 = mysqli_query($conn, 'SELECT samedi FROM graphique WHERE idPoubelle=1');
  $donnees6 = mysqli_fetch_array($req6) ; 
  $nombreDeSacSamedi = $donnees6['samedi'];

  $req7 = mysqli_query($conn, 'SELECT dimanche FROM graphique WHERE idPoubelle=1');
  $donnees7 = mysqli_fetch_array($req7) ; 
  $nombreDeSacDimanche = $donnees7['dimanche'];
  ?>
...
"https://cdnjs.cloudflare.com/ajax/libs/Chart.js/2.2.2/Chart.min.js"></script>
  ...
  <script>
  var nombreDeSacLundi = <?php echo json_encode($nombreDeSacLundi); ?>
  var nombreDeSacMardi = <?php echo json_encode($nombreDeSacMardi); ?>
  var nombreDeSacMercredi = <?php echo json_encode($nombreDeSacMercredi); ?>
  var nombreDeSacJeudi = <?php echo json_encode($nombreDeSacJeudi); ?>
  var nombreDeSacVendredi = <?php echo json_encode($nombreDeSacVendredi); ?>
  var nombreDeSacSamedi = <?php echo json_encode($nombreDeSacSamedi); ?>
  var nombreDeSacDimanche = <?php echo json_encode($nombreDeSacDimanche); ?>
        ...
        datasets: [
          {
            ...
            data: [nombreDeSacLundi,nombreDeSacMardi ,  nombreDeSacMercredi,  nombreDeSacJeudi,  nombreDeSacVendredi,  nombreDeSacSamedi,  nombreDeSacDimanche],
